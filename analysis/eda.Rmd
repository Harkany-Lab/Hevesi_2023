...
plot1 <- QC_Plot_GenevsFeature(seurat_object = combined.srt, feature1 = "percent_mito", low_cutoff_gene = low.cutoff.gene, high_cutoff_gene = high.cutoff.gene, high_cutoff_feature = high.cutoff.pc.mt, color_seed = reseed, ggplot_default_colors = T) & scale_y_log10()
plot2 <- QC_Plot_UMIvsGene(seurat_object = combined.srt, low_cutoff_gene = low.cutoff.gene, high_cutoff_gene = high.cutoff.gene, low_cutoff_UMI = low.cutoff.umis, high_cutoff_UMI = high.cutoff.umis, color_seed = reseed, ggplot_default_colors = T) & scale_x_log10() & scale_y_log10()
plot3 <- QC_Plot_GenevsFeature(seurat_object = combined.srt, feature1 = "percent_ribo", low_cutoff_gene = low.cutoff.gene, high_cutoff_gene = high.cutoff.gene, high_cutoff_feature = high.cutoff.pc.ribo, color_seed = reseed, ggplot_default_colors = T) & scale_y_log10()
plot4 <- FeatureScatter(combined.srt, feature1 = "percent_ribo", feature2 = "percent_mito", shuffle = T)
(plot1 + plot2) / (plot3 + plot4)
```

```{r pl-scatter-qc-comb, fig.height=7, fig.width=13, fig.align='center'}
QC_Plot_UMIvsGene(seurat_object = combined.srt, meta_gradient_name = "percent_mito", low_cutoff_gene = low.cutoff.gene, high_cutoff_gene = high.cutoff.gene, high_cutoff_UMI = high.cutoff.umis, meta_gradient_low_cutoff = high.cutoff.pc.mt, combination = TRUE, color_seed = reseed, ggplot_default_colors = T) & scale_x_log10() & scale_y_log10()
```
...
if (any(sum(combined.srt$cell_name %in% souporcell$cell_name))) {
  ...
  FeatureScatter(combined.srt, feature1 = "nCount_RNA", feature2 = "log_prob_doublet", shuffle = T)
} else {
...
combined.srt <- 
  FindVariableFeatures(
    ...
    nfeatures = 3000)
top20 <- head(VariableFeatures(combined.srt), 20)
top20
plot5 <- VariableFeaturePlot(combined.srt)
LabelPoints(plot = plot5, points = top20, repel = TRUE, xnudge = 0, ynudge = 0)
...
p1 <- DimPlot_scCustom(combined.srt, label = T, repel = T, pt.size = 2) + ggtitle("Unsupervised clustering") + NoLegend()
...
FeaturePlot_scCustom(combined.srt, features = "percent_mito", colors_use = combined.srt@misc$mdat_Colour_Pal) & 
  theme(plot.title = element_text(size = 16))

FeaturePlot_scCustom(combined.srt, features = "percent_mito", colors_use = combined.srt@misc$mdat_Colour_Pal, na_cutoff = 5)

FeaturePlot_scCustom(combined.srt, features = "nFeature_RNA", colors_use = combined.srt@misc$mdat_Colour_Pal) & 
  theme(plot.title = element_text(size = 16))
```
...
FeaturePlot_DualAssay(seurat_object = combined.srt, features = "Galr1", assay1 = "RNA", assay2 = "RAW", colors_use = combined.srt@misc$expr_Colour_Pal) & 
  theme(plot.title = element_text(size = 24))
```

```{r echo=FALSE, fig.height=4, fig.width=10, fig.align='center'}
FeaturePlot_DualAssay(seurat_object = combined.srt, features = "Gal", assay1 = "RNA", assay2 = "RAW", colors_use = combined.srt@misc$expr_Colour_Pal) & 
  theme(plot.title = element_text(size = 24))
```
...
```{r fig.align='center', fig.asp=.618, fig.width=5}
FeaturePlot_scCustom(combined.srt,features = "percent_mito", label.size = 4, repel = T, pt.size = 3, label = T, colors_use = combined.srt@misc$mdat_Colour_Pal)  & 
  theme(plot.title = element_text(size = 16))
```
...
FeaturePlot_scCustom(combined.srt, features = "percent_ribo",
            label.size = 4,repel = T,pt.size = 3,label = T, colors_use = combined.srt@misc$mdat_Colour_Pal)  & 
  theme(plot.title = element_text(size = 16))
```
...
FeaturePlot_scCustom(combined.srt, "Galr1", pt.size = 2, order = T, colors_use = combined.srt@misc$expr_Colour_Pal) + 
  ggtitle("Galr1: ") + theme(plot.title = element_text(size = 24))
```

```{r fig.align='center', fig.width=5, fig.asp = 0.618}
FeaturePlot_scCustom(combined.srt, "Gal", pt.size = 2, order = T, colors_use = combined.srt@misc$expr_Colour_Pal) + 
  ggtitle("Gal: ") + theme(plot.title = element_text(size = 24))
```

```{r, fig.asp=.2, fig.width=16}
DotPlot_scCustom(seurat_object = combined.srt, assay = "RNA", features = c("Galr1", "Gal"), flip_axes = T, x_lab_rotate = TRUE, split.by = "orig.ident")
```
...
top5_markers <- Extract_Top_Markers(marker_dataframe = markers.logreg, num_genes = 5, named_vector = FALSE, make_unique = TRUE)
...
```{r pl-heatmap, fig.width = 18, fig.asp = 1.1}
markers.logreg %>%
    group_by(cluster) %>%
...
```{r pl-heatmap-THP7, fig.width = 18, fig.asp = 1.1}
n_pcs <- 100
...
```{r include=FALSE, fig.align='center', fig.height=10, fig.width=10}
THP7.top5_markers <- Extract_Top_Markers(marker_dataframe = THP7.markers.logreg, num_genes = 5, named_vector = FALSE, make_unique = TRUE)
...
```{r pl-heatmap-Pr5P7, fig.width = 18, fig.asp = 1.1}
n_pcs <- 100
...
```{r include=FALSE, fig.align='center', fig.height=10, fig.width=10}
Pr5P7.top5_markers <- Extract_Top_Markers(marker_dataframe = Pr5P7.markers.logreg, num_genes = 5, named_vector = FALSE, make_unique = TRUE)
